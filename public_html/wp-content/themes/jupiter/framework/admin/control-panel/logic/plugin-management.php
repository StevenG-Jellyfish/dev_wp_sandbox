...
class mk_plugin_management {
	...
	public function install() {
		...
		try {
			...
			if ( filter_var( $api_response[0]['source'], FILTER_VALIDATE_URL ) === false ) {
				throw new Exception( 'Plugins source not found or it have invalid url' );
			}
	...
	public function update_plugin() {
		try {
			...
			$this->set_lock( 'update_plugin' , false )->message( 'Plugin successfully updated.', true );
			return true;
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function plugins_list_from_api( $from = 0, $count = 1, $exclude_plugins = array() ) {
		...
		if ( isset( $response->body->bool ) == false || $response->body->bool == false ) {
			$this->message( $response->body->sys_msg, false );
			return false;
		}
	...
	public function deactivate_plugin( $plugin_slug ) {
		$plugin_path = $this->find_plugin_path( $plugin_slug );
		if ( $plugin_path === false ) {
			throw new Exception( 'Can not find plugin path in deactivate plugin func' );
			return false;
		}
